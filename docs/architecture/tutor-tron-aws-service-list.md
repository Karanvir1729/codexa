# Tutor-Tron AWS Architecture Service List

| Component | Runs on | Purpose |
|---|---|---|
| React web app | User device/browser | Lesson UI, WebRTC setup, local event capture. |
...
| EC2 G5/G6 vLLM tutor model | Real-time backend model tier | Fast open-weight reasoning model. |
...
| Amazon Keyspaces / Cassandra ring | Distributed storage | RF=3 cohort/student/strategy memory with local quorum style reliability. |
| Qdrant shards | Distributed vector layer | Curriculum, misconception, student memory, cohort, eval, and strategy vectors. |
...
| Async worker ASG | Async worker layer | Student state, pitfall, RAG, similar-student, SCIP, eval, and strategy promotion workers. |
