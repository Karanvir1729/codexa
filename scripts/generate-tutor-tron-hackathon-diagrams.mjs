...
const sourcePath = "docs/architecture/tutor-tron-feedback-state-machine.excalidraw";
const judgeOut = "docs/architecture/tutor-tron-hackathon-architecture.excalidraw";
const judgeClip = "docs/architecture/tutor-tron-hackathon-architecture.clipboard.json";
const appendixOut = "docs/architecture/tutor-tron-technical-appendix.excalidraw";
const appendixClip = "docs/architecture/tutor-tron-technical-appendix.clipboard.json";
...
function buildJudgeDiagram() {
  ...
  text(a, 55, 34, 1060, "Tutor-Tron Voice Agent: Modular Production Architecture", 39, C.ink, { align: "left" });
  text(
    a,
    60,
    84,
    1130,
    "Production-grade voice tutor: live tutoring stays fast while async agents learn from failures,\nrun evals, and safely promote better teaching strategies.",
    18,
    "#334155",
    { align: "left" },
  );
  ...
  lane(
    ...
    C.orange,
    C.orangeBg,
    "Two entry paths converge into the same colocated realtime tutoring pipeline.",
  );
  ...
  card(a, 1030, 212, 325, 88, "Cloud Run Session Router", "tokens, stickiness,\nregion hint", C.blue, "#ffffff", { fileId: "file_nlb", iconSize: 42, titleSize: 17 });
  card(a, 1398, 212, 430, 88, "Local Lightweight Models", "VAD | interruption detector | phrase cache\noptional WebGPU misconception classifier", C.orange, "#ffffff", { fileId: "file_webgpu", iconSize: 42, titleSize: 17 });
  ...
  lineLabel(a, 94, 560, 520, "1. Student speaks through browser/WebRTC or phone/Twilio", C.red, 14);
  ...
  lineLabel(a, 820, 545, 420, "2. Realtime tutor responds without waiting for async workers", C.red, 14);
  ...
  card(a, 82, 695, 430, 58, "Redis Streams Event Bus", "transcript.final | pitfall.detected | strategy.failed\neval_case.generated | strategy.promoted", C.blue, "#ffffff", { fileId: "file_redis", iconSize: 32, titleSize: 15, bodySize: 9 });
  card(a, 548, 695, 310, 58, "Session + Student State", "concept | mastery | active misconception | hint history", C.blue, "#ffffff", { fileId: "file_cache", iconSize: 32, titleSize: 15, bodySize: 10 });
  card(a, 894, 682, 330, 84, "Active Teaching Strategy Cache", "only promoted strategy versions\nused by live tutor", C.green, "#ffffff", { fileId: "file_cache", iconSize: 42, titleSize: 15, bodySize: 11 });
  ...
  card(a, 92, 898, 270, 78, "Student State + Pitfall", "mastery, confusion,\nmisconception tags", C.teal, "#ffffff", { fileId: "file_worker", iconSize: 40, titleSize: 15, bodySize: 11 });
  ...
  lineLabel(a, 710, 1000, 360, "SCIP: next tutoring action under latency + confidence constraints", C.teal, 13);
  ...
  lane(
    ...
    1880,
    292,
    "5. SELF-IMPROVEMENT LOOP: Student Failure -> Promoted Teaching Strategy",
    C.green,
    C.greenBg,
  ...
  const loop = [
    ...
    ["Pitfall Classified", "misconception tag"],
  ...
  lineLabel(a, 1588, 1266, 260, "8. Next similar student gets improved explanation", C.greenDark, 13);
  card(a, 72, 1268, 430, 60, "Learning Outcome Metrics", "follow-up correctness | fewer hints | misconception resolved\nlatency | confidence | rubric score", C.green, "#ffffff", { titleSize: 15, bodySize: 10 });
  ...
  text(a, 70, 1702, 940, "Hybrid-cloud rule: realtime voice path stays colocated.\nAsync eval/replay workers can run cross-cloud because they do not block live tutoring.", 14, C.gray, { align: "left" });
...
function buildTechnicalAppendix() {
  counter = 0;
  const a = [];
  text(a, 55, 34, 970, "Tutor-Tron Technical Appendix: Node-Level Prototype Architecture", 36, C.ink, { align: "left" });
  ...
  for (let i = 0; i < 3; i += 1) {
    const x = 90 + i * 580;
    card(a, x, 1028, 510, 96, `Async Agent Node ${i + 1}`, "Student State | Pitfall | RAG | Similar Student | SCIP | Eval Generation | Auto-Improvement", C.teal, "#ffffff", { fileId: "file_worker", iconSize: 46, titleSize: 17, bodySize: 11 });
  ...
  card(a, 90, 1140, 340, 40, "HTTP: /internal/turns/plan", "low-latency tutor action request", C.gray, "#ffffff", { titleSize: 13, bodySize: 10 });
  ...
  card(a, 1160, 1650, 300, 86, "Dashboard Proof", "transcript | pitfall | strategy | eval before/after | promoted version", C.green, "#ffffff", { fileId: "file_grafana", iconSize: 44 });
