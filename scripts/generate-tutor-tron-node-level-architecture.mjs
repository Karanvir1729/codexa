...
const outPath = "docs/architecture/tutor-tron-node-level-architecture.excalidraw";
const clipPath = "docs/architecture/tutor-tron-node-level-architecture.clipboard.json";
...
// Title.
a.push(text(60, 34, 980, "Tutor-Tron Voice Agent", 46, C.ink, { align: "left" }));
a.push(text(65, 92, 820, "Fast live tutoring + async eval-gated strategy promotion.", 22, "#334155", { align: "left" }));
...
// Layer 1.
lane(a, 45, 145, 1880, 210, "1. USER DEVICE + DAILY / TWILIO INGRESS", C.orange, C.orangeBg, "Two entry paths: browser sessions use WebRTC/Daily; phone tutoring uses Twilio. Both stay colocated with the same realtime voice pipeline.");
card(a, 75, 225, 185, 90, "React Client", "lesson UI\nstudent profile", C.orange, "#ffffff", { fileId: "file_browser" });
card(a, 285, 225, 185, 90, "WebRTC Mic", "Daily media\nlow jitter", C.orange, "#ffffff", { fileId: "file_mic" });
card(a, 495, 225, 230, 90, "Local Signal Models", "VAD | interruption\noptional misconception", C.orange, "#ffffff", { fileId: "file_webgpu" });
...
// Layer 2.
lane(a, 45, 385, 1880, 270, "2. LOW-LATENCY REALTIME VOICE LAYER: DAILY / PIPECAT / TWILIO", C.red, C.redBg, "Do not split this path across clouds: Student -> WebRTC/Twilio -> Pipecat -> ASR -> LLM -> TTS -> Student.");
...
label(a, 80, 360, 280, "Browser/WebRTC student session", C.red, 13, { align: "left" });
...
a.push(text(840, 718, 780, "Events: transcript.final | pitfall.detected | strategy.failed | eval_case.generated | strategy.promoted | strategy.rollback", 14, C.blue, { align: "left" }));
...
card(a, 1570, 745, 285, 50, "Active Teaching Strategy Cache", "only promoted versions", C.green, "#ffffff", { fileId: "file_cache", iconSize: 34, titleSize: 14, bodySize: 11 });
...
card(a, 90, 925, 330, 130, "Student State Agent", "mastery estimate\nmisconception / pitfall detector\nconfusion + frustration signal", C.teal, "#ffffff", { fileId: "file_worker", iconSize: 52, titleSize: 19 });
card(a, 460, 925, 330, 130, "RAG Retrieval Agent", "NVIDIA embed/rerank\nsimilar failures\nconcept + pitfall memory", C.teal, "#ffffff", { fileId: "file_rag", iconSize: 52, titleSize: 19 });
card(a, 830, 925, 430, 130, "SCIP Optimization Layer", "selects hint, analogy, quiz, re-teach\nunder latency + confidence constraints", C.teal, "#ffffff", { fileId: "file_scip", iconSize: 52, titleSize: 18 });
...
label(a, 105, 1080, 300, "student_state.updated", C.blue, 13, { align: "left" });
...
lane(a, 45, 1185, 1880, 325, "4. CEKURA-STYLE SELF-IMPROVEMENT LOOP: Failure -> Promoted Teaching Strategy", C.green, C.greenBg, "Simulation, replay, regression testing, monitoring, and gated promotion. The tutor improves by promoting tested strategy versions, not by live fine-tuning.");
const stateY = 1275;
const stateW = 205;
const stateGap = 22;
const states = [
...
a.push(text(70, 1928, 1050, "Async eval/replay workers can move clouds because they do not block live tutoring.", 15, C.gray, { align: "left" }));
